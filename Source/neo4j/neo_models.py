...
class KGneo4j():
	...
	def find_relation_by_node_one(self,entity1):
		sql = "MATCH (e1:term)-[r:rel]->(e2:term) WHERE e1.name=\"" +str(entity1)+"\" RETURN e1,r,e2"
		answer = self.graph.run(sql).data()
		return answer

	#查找entity2及其对应的关系
	def find_relation_by_node_two(self,entity2):
		sql = "MATCH (e1:term)-[r:rel]->(e2:term) WHERE e2.title=\"" +str(entity2)+"\" RETURN e1,r,e2"
		answer = self.graph.run(sql).data()
		return answer

	#根据entity1和关系查找enitty2
	def find_other_node(self,entity,relation):
		sql = "MATCH (n1:term{name:\"" + str(entity) + "\"})- [r:rel{relation:\""+str(relation)+"\"}]->(n2) RETURN n2"
		answer = self.graph.run(sql).data()
		return answer
	...
	def find_shotest_path(self,entity1,entity2):
		sql = "MATCH (n1:term{name:\"" + str(entity1) + "\"}),(n2:term{name:\""+str(entity2)+"\"}),p=shortestpath((n1)-[r:rel*]-(n2)) RETURN p"
		answer = self.graph.run(sql).data()
		#self.parse_answer(answer)
		relationDict = []
		for x in answer:
			retpath = x['p']
			for rel in retpath.relationships:
				print(rel)
			#s = retpath.start_node
			#e = retpath.end_node
			#print(s, e)
			#print(ret.start_node)
			#print(ret.start_node)
