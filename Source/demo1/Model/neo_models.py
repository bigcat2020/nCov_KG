...
class Neo4j():
	...
	# 根据entity的名称返回是否有此节点（author kissss; date 2020-03-19; time 1：09）
	def getEntityExist(self,value):
		#answer = self.graph.run("MATCH (entity1) - [rel] -> (entity2)  WHERE entity1.title = \"" +str(value)+"\" RETURN rel,entity2").data() #old
		answer = self.graph.run("MATCH (entity) WHERE entity.name = \"" +str(value)+"\" RETURN entity").data() #new
		return answer

	# 根据entity的名称返回关系
	def getEntityRelationbyEntity(self,value):
		#answer = self.graph.run("MATCH (entity1) - [rel] -> (entity2)  WHERE entity1.title = \"" +str(value)+"\" RETURN rel,entity2").data() #old
		answer = self.graph.run("MATCH (entity1) - [rel] -> (entity2)  WHERE entity1.name = \"" +str(value)+"\" RETURN rel,entity2").data() #new
		return answer
