...
def search_entity(request):
	...
	if(request.GET):
		...
		#连接数据库
		db = neo_con
		entityRelation = db.getEntityRelationbyEntity(entity) #存在问题，如果node存在，但node无relation导致返回未收录
		# print(entityRelation[0])							  #已经在neo_models添加getEntityExist函数，接下来需要添加判断
		if len(entityRelation) == 0:						  # by kissss
			#若数据库中无法找到该实体，则返回数据库中无该实体
			ctx= {'title': '<h1>数据库中暂未添加该实体</h1>'}
			return render(request, 'entity.html',{'ctx': json.dumps(ctx,ensure_ascii=False),'entity':request.GET['user_text']})
		else:
			#返回查询结果
			for i in range(len(entityRelation)):
				relationName = entityRelation[i]['rel']['relation']
				entity2Name = entityRelation[i]['entity2']['name']
				entityRelation[i] = [relationName, entity2Name]
			...
			entityRelation = []
			for i in dict.items():
				entityRelation.append(i[0])
				entityRelation.append(i[1])
			print(request.GET['user_text'])
			return render(request, 'entity.html', {'entityRelation': entityRelation, "entity": request.GET['user_text']})
