        ...
        for line in f:
            try:
                ...
                if content:
                    ...
                    labels.append(int(native_content(label)))
            except:
                pass
...
def process_str( txt, word_to_id, max_length=20):
    #处理一个字符串，作为x输入矩阵
    content = list(native_content(txt))
...
import csv
def load_vocab(train_dir, vocab_dir, vocab_size=5000):
    contents = list()
...
#build_vocab('./train.txt', './vocab.txt', vocab_size=5000)