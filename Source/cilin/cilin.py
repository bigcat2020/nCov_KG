...
class CilinSimilarity(object):
    ...
    def __init__(self):
        ...
        self.word_code = {}
        self.vocab = set()
        self.file = './new_cilin_utf8.txt'
        self.synonymlist = list()
        self.mydict = {}
        self.read_cilin()
        self.read_user_synonym()
    ...
    def read_user_synonym(self):
        self.synonymlist = list()
        with open('./user_synonym.txt','r',encoding='utf-8') as f:
            for line in f.readlines():
                res = line.split(',')
                self.synonymlist.append(res)
        #print(self.synonymlist)
    
    def get_user_synonym( self, word ):
        for res in self.synonymlist:
            for w in res:
                if w == word:
                    return res
        return []
    ...
    def get_synonym(self, word):
        #查找一个词语的同义词
        user_syn = set()
        user_syn.update(self.get_user_synonym(word))
        if word in self.word_code:
            keys = self.word_code[word]
            cilin_syn = list()
            for k in keys:
                cilin_syn = cilin_syn + self.code_word[k][:5] #只获取前5个相关词，其他差异太大
        user_syn.update(cilin_syn)
        result = list()
        for w in user_syn:
            #sim = self.sim2018(w, word)
            #if sim>0.98:
            result.append(w)
        return result
...
print(cilin.sim2018('发布','发表'))
print(cilin.sim2018('收回','发表'))
#print(cilin.get_synonym('武汉'))
print(cilin.get_synonym('事件'))
print(cilin.get_synonym('参与'))
