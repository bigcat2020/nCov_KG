...
class WeiSpider:
    ...
    def writer_soup(self, soup):
        with open("..\\..\\data\\weibo\\" + self.word + ".txt", "ab") as fp:
            list_soup = soup.find_all(name='div', attrs={"class": "c"})
            for i in list_soup[6::]:
                try:
                    #print( i )
                    print("-" * 100 + "\n")
                    txt = self.clean_text( i.a.text )
                    url = i.get('href')
                    print(url)
                    if type(url)=='str' and len(url)>0:
                        txt = url + ',' + self.clean_text(url)
                        print( url )
                    content = i.find(name="span", attrs={"class": "ctt"}).text
...
def main():
    words = pd.read_csv('..\\..\\data\\keywords.csv', encoding='utf-8', index_col=0)
    #word = input("请输入查询关键字：")
    #page = input("请输入页数：")
    page = 30
    for i in range(1, words.shape[0]):
        word = words.index[i-1]
        spider = WeiSpider(word, page + random.randint(0,9) )
        spider.run_spider()
