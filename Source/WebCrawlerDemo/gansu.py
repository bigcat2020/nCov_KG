...
class downloader():
	def __init__(self):
		self.server = 'wsjk.gansu.gov.cn' #网站域名
		self.main_url = 'http://wsjk.gansu.gov.cn' #文章网站的前缀url 会因网站不同而不同
		self.list_url = ['http://wsjk.gansu.gov.cn/channel/11218/index.html'] #疫情发布的列表主页，用它来获取文章
	...
	def get_artcles_url(self):
		for i in self.list_url:
			...
			div = div_bf.find_all('div',class_='m_new_list_detail')
			a_bf = BeautifulSoup(str(div),'lxml')
			a = a_bf.find_all('a')
			for x in a:
				print(x.get('href'),x.text)
				self.articles_url.append(x.get('href'))
			name_bf = BeautifulSoup(str(div),'lxml')
			name = name_bf.find_all('em')
			for y in name:
				self.articles_name.append(y.text)
	...
	def writer(self):
		...
		for i in range(url_count):
			...
			div_bf = BeautifulSoup(request,'lxml')
			div = div_bf.find_all('div',id='contents') #获取div标签
			inner = ""
			for j in div:
				inner += '\n' + j.text
			try:
