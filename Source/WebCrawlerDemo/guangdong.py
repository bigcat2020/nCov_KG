...
class downloader():
	def __init__(self):
		...
		self.list_url = ['http://wsjkw.gd.gov.cn/xxgzbdfk/yqtb/',\
		'http://wsjkw.gd.gov.cn/xxgzbdfk/yqtb/index_2.html'
		] #疫情发布的列表主页，用它来获取文章
	...
	def get_artcles_url(self):
		for i in self.list_url:
			...
			div = div_bf.find_all('div',class_='section list')
	...
	def writer(self):
		...
		for i in range(url_count):
			...
			div_bf = BeautifulSoup(request,'lxml')
			div = div_bf.find_all('div',class_='content-content') #获取div标签
			inner = ""
			for j in div:
				inner += '\n' + j.text
			try:
