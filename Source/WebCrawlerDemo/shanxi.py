...
class downloader():
	def __init__(self):
		self.server = 'sxwjw.shaanxi.gov.cn' #网站域名
		self.main_url = 'http://sxwjw.shaanxi.gov.cn' #文章网站的前缀url 会因网站不同而不同
		self.list_url = ['http://sxwjw.shaanxi.gov.cn/col/col9/index.html?uid=572&pageNum=1',\
		'http://sxwjw.shaanxi.gov.cn/col/col9/index.html?uid=572&pageNum=2',\
		'http://sxwjw.shaanxi.gov.cn/col/col9/index.html?uid=572&pageNum=3'
		] #疫情发布的列表主页，用它来获取文章
	...
	def get_artcles_url(self):
		for i in self.list_url:
			...
			div = div_bf.find_all('div',class_='default_pgContainer')
	...
	def writer(self):
		...
		for i in range(url_count):
			...
			div_bf = BeautifulSoup(request,'lxml')
			div = div_bf.find_all('div',id='zoom') #获取div标签
			inner = ""
			for j in div:
				inner += '\n' + j.text
			try:
