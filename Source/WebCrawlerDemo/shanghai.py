...
class downloader():
	def __init__(self):
		self.server = 'wsjkw.sh.gov.cn' #网站域名
		self.main_url = 'http://wsjkw.sh.gov.cn' #文章网站的前缀url 会因网站不同而不同
		self.list_url = ['http://wsjkw.sh.gov.cn/xwfb/index.html',\
		'http://wsjkw.sh.gov.cn/xwfb/index_2.html',\
		'http://wsjkw.sh.gov.cn/xwfb/index_3.html',\
		'http://wsjkw.sh.gov.cn/xwfb/index_4.html',\
		'http://wsjkw.sh.gov.cn/xwfb/index_5.html'
		] #疫情发布的列表主页，用它来获取文章
	...
	def get_artcles_url(self):
		for i in self.list_url:
			...
			div = div_bf.find_all('ul',class_='uli16 nowrapli list-date ')
	...
	def writer(self):
		...
		for i in range(url_count):
			...
			div_bf = BeautifulSoup(request,'lxml')
			div = div_bf.find_all('div',class_='Article_content') #获取div标签
			inner = ""
			for j in div:
				inner += '\n' + j.text
			try:
