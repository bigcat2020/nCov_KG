...
class downloader():
	def __init__(self):
		self.server = 'www.gzhfpc.gov.cn' #网站域名
		self.main_url = 'http://www.gzhfpc.gov.cn/ztzl_500663/xxgzbdgrdfyyqfk/yqdt' #文章网站的前缀url 会因网站不同而不同
		self.list_url = ['http://www.gzhfpc.gov.cn/ztzl_500663/xxgzbdgrdfyyqfk/yqdt/',\
		'http://www.gzhfpc.gov.cn/ztzl_500663/xxgzbdgrdfyyqfk/yqdt/index_1.html'] #疫情发布的列表主页，用它来获取文章
	...
	def get_artcles_url(self):
		for i in self.list_url:
			...
			div = div_bf.find_all('div',class_='right_list f_r f14')
			dd_bf = BeautifulSoup(str(div),'lxml')
			dd = dd_bf.find('dd')
			a_bf = BeautifulSoup(str(dd),'lxml')
			a = a_bf.find_all('a')
			for x in a:
				print(x.get('href'),x.text)
				self.articles_url.append(self.main_url + x.get('href').strip('.'))
				self.articles_name.append(x.get('title'))
			date_bf = BeautifulSoup(str(a),'lxml')
			date = date_bf.find_all('i')
			for z in date:
				print(z.text)
	...
	def writer(self):
		...
		for i in range(url_count):
			...
			div_bf = BeautifulSoup(request,'lxml')
			div = div_bf.find_all('div',class_='content') #获取div标签
			inner = ""
			for j in div:
				inner += '\n' + j.text
			try:
