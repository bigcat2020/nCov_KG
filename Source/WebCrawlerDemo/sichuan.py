...
class downloader():
	def __init__(self):
		self.server = 'wsjkw.sc.gov.cn' #网站域名
		self.main_url = 'http://wsjkw.sc.gov.cn' #文章网站的前缀url 会因网站不同而不同
		self.list_url = ['http://wsjkw.sc.gov.cn/scwsjkw/gzbd01/ztwzlmgl.shtml',\
	...
	def get_artcles_url(self):
		for i in self.list_url:
			...
			div = div_bf.find_all('div',class_='contMain fontSt')
	...
	def writer(self):
		...
		for i in range(url_count):
			...
			div_bf = BeautifulSoup(request,'lxml')
			div = div_bf.find_all('div',class_='wy_contMain fontSt') #获取div标签
			inner = ""
			for j in div:
				inner += '\n' + j.text
			try:
