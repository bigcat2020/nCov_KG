...
class downloader():
	def __init__(self):
		self.server = 'wsjkw.gd.gov.cn' #网站域名
		self.main_url = 'http://www.hnwsjsw.gov.cn' #文章网站的前缀url 会因网站不同而不同
		self.list_url = ['https://wsjkw.qinghai.gov.cn/ztbd/yqjk/yqtb/index.html',\
		'https://wsjkw.qinghai.gov.cn/ztbd/yqjk/yqtb/index1.html',\
		'https://wsjkw.qinghai.gov.cn/ztbd/yqjk/yqtb/index2.html'
		] #疫情发布的列表主页，用它来获取文章
	...
	def get_artcles_url(self):
		for i in self.list_url:
			...
			li_bf = BeautifulSoup(request,'lxml')
			li = li_bf.find_all('li',class_='line01')
			a_bf = BeautifulSoup(str(li),'lxml')
			a = a_bf.find_all('a',target='_blank')
			for x in a:
				print(x.get('href'),x.text)
				self.articles_url.append(x.get('href'))
				self.articles_name.append(x.text.replace('\n','').replace('\t',''))
			date_bf = BeautifulSoup(str(li),'lxml')
	...
	def writer(self):
		...
		for i in range(url_count):
			...
			div = div_bf.find_all('tr') #获取div标签
			inner = div[1].text
			try:
