...
"""
Bi-LSTM + Attention 就是在Bi-LSTM的模型上加入Attention层，在Bi-LSTM中我们会用最后一个时序的输出向量
作为特征向量，然后进行softmax分类。Attention是先计算每个时序的权重，然后将所有时序 的向量进行加权和作
为特征向量，然后进行softmax分类。在实验中，加上Attention确实对结果有所提升。
网络层次为：
输入层：X1 ... Xt
Embedding层：E1 ... Et
双向LSTM层：h1->...->ht，h1<-...<-ht
...
算法论文：
Peng Zhou, Wei Shi, Jun Tian，Attention-Based Bidirectional Long Short-Term Memory Networks for Relation Classification

数据集总共有三个数据文件，在/data/rawData目录下，
包括unlabeledTrainData.tsv，labeledTrainData.tsv，testData.tsv。
在进行文本分类时需要有标签的数据（labeledTrainData），
预处理后的文件为/data/preprocess/labeledTrain.csv
"""
...
#训练模型时，定义了tensorBoard输出，并定义了两种模型保存的方法。
# 生成训练集和验证集
