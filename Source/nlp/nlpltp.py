...
def test_plt( pltobj, s ):
    ...
    #roles = plt.role_label(words, postags, arcs)
...
if True:
    test_plt( plt, '塞内加尔出现首例输入性新冠肺炎患者，该病患是一名常住塞内加尔首都达喀尔市阿勒马蒂区的法国人。' )
    #test_plt( plt, '元芳你怎么看' )
    test_plt( plt, '李兰娟院士赞扬陕西省援助湖北医疗队。' )
    #test_plt( plt, '据俄罗斯卫星通讯社3日消息，世卫组织在伊朗的一名工作人员被确诊感染新型冠状病毒，病情目前处于中等程度。')
else:
    with open( 'd:\\report.txt', 'r', encoding='utf-8' ) as f:
        lines = f.readlines()
        for text in lines:
            for s in plt.sentence(text):
                print( '处理句子:', ''.join(s) )
                test_plt( plt, ''.join(s) )
